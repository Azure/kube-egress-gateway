...
func init() {
	...
	rootCmd.Flags().IntVar(&lbProbeDrainDelaySeconds, "lb-probe-drain-delay-seconds", 30, "Seconds to wait after marking LB probe unhealthy before shutting down (allows LB to drain traffic).")
...
func startControllers(cmd *cobra.Command, args []string) {
	...
	lbProbeServer := healthprobe.NewLBProbeServer(gatewayLBProbePort, time.Duration(lbProbeDrainDelaySeconds)*time.Second)
